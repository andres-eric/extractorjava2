...
public interface ExtranctorRepository extends JpaRepository<consulta, consultaId> {
    ...
    @Query(value = """
            ...
            WITH tabla_inicial AS (
                ...
                SELECT *,
                    CASE
                        WHEN origen_proveedor = 'NACIONAL' THEN COALESCE(TRNDTE_10, CURPRM_10, TNXDTE_15)
                        ELSE COALESCE(CURPRM_10, TRNDTE_10, TNXDTE_15)
                    END AS fecha,
                    CASE
            ...
            , DIAS AS (
                ...
                SELECT
                    ...
                    f.PORCENTAJE_CUMPLIMIENTO_PRIMERA_ENTREGA AS porcentaje_cumplimiento_primera_entrega, /* === siempre tiene que ir asi con ese as========= */
                    k.mes,
                    k.anio
                FROM tabla_kpi AS k
